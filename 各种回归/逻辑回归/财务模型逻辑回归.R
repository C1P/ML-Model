...
b1[which(a2$速动比率... < 247.42 & a2$资产负债率... > 45.305
         & a2$利息保障倍数.倍. < 52.9 & a2$带息负债比率... > 33.3
         & a2$盈余现金保障倍数.倍. < 0 & a2$营业利润增长率...  < 211.3
         & a2$总资产周转率.次. < 0.7)] <- 1
...
ggplot(plot1) + geom_point(aes(x = pfr,y = ptr,col = "red")) + 
    geom_line(aes(pl,pl),col = "blue") + ggtitle("ROC") + xlab("PFR") + ylab("PTR")
